...
import argparse
from .pipeline_utils import construct_full_experiment_id, normalize_to_8bit_for_display
from .file_paths import (
    PROJECT_ROOT,
    IMAGE_DIR_RELATIVE_TO_PROJECT,
...
# PROJECT_ROOT is imported
# IMAGE_DIR_PATH = os.path.join(PROJECT_ROOT, IMAGE_DIR_BASE)
RESULTS_DIR_PATH_DEFAULT = os.path.join(PROJECT_ROOT, RESULTS_DIR_RELATIVE_TO_PROJECT)
...
def create_segmentation_summary_with_consistency_coloring(
    all_image_configs, 
    all_cellpose_params, 
    cmd_args
    ):
    ...
    effective_results_dir = cmd_args.results_dir
    ...
    for img_config in active_image_configs:
        ...
        rescaling_config = segmentation_options.get("rescaling_config", {})
        tiling_params = segmentation_options.get("tiling_parameters", {})

        is_tiled = tiling_params.get("apply_tiling", False)
        is_rescaled = rescaling_config.get("apply_rescaling", False) and rescaling_config.get("output_scale_factor") != 1.0

        if is_tiled or is_rescaled:
            print(f"Skipping image_id '{image_id}' for summary: Tiled or rescaled images are not currently supported by this summary script.")
            continue
        ...
        if original_image_relative_path not in original_images_cache:
            if os.path.exists(original_image_full_path):
                try:
                    ...
                    original_images_cache[original_image_relative_path] = normalize_to_8bit_for_display(orig_img_cv)
                except Exception as e:
        ...
        processing_unit_filename_stem = os.path.splitext(os.path.basename(original_image_relative_path))[0]
        ...
        for cp_config in active_cellpose_params:
            ...
            current_experiment_id = construct_full_experiment_id(image_id, param_set_id, scale_factor=1.0, tile_info=None)
            
            mask_filename = f"{processing_unit_filename_stem}_mask.tif"
            mask_filepath = os.path.join(effective_results_dir, current_experiment_id, mask_filename)
            ...
            if os.path.exists(mask_filepath):
                try:
                    ...
                    print(f"Error loading mask {mask_filepath} for {current_experiment_id}: {e}. Skipping.")
            else:
                print(f"Mask file not found for {current_experiment_id} at {mask_filepath}. Skipping.")

    if not all_masks_data or mask_shape is None:
        print("No valid segmentation masks found (from non-tiled/non-rescaled active configurations) or mask_shape not determined for summary.")
        return
    ...
    for i, data in enumerate(plot_data): 
        ...
        if img_to_show_under_mask.ndim == 3 and img_to_show_under_mask.shape[-1] == 3: 
            pass
        elif img_to_show_under_mask.ndim == 2: 
            pass
            ...
            if img_to_show_under_mask.dtype != np.uint8 : img_to_show_under_mask = normalize_to_8bit_for_display(img_to_show_under_mask)
    ...
    final_output_summary_image_path = os.path.join(cmd_args.results_dir, cmd_args.output_filename)
    summary_output_dir = os.path.dirname(final_output_summary_image_path)
    ...
    try:
        plt.savefig(final_output_summary_image_path, dpi=120)
        print(f"Summary image with consistency colors saved to: {final_output_summary_image_path}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default=os.path.join(PROJECT_ROOT, DEFAULT_CONFIG_FILENAME),
                        help=f"Path to the processing JSON configuration file (default: {DEFAULT_CONFIG_FILENAME} in project root).")
    parser.add_argument("--results_dir", default=RESULTS_DIR_PATH_DEFAULT,
                        help=f"Base directory where segmentation results (experiment folders with masks) are stored (default: '{RESULTS_DIR_RELATIVE_TO_PROJECT}' in project root).")
    ...
    print(f"Results will be read from: {args.results_dir}")
    print(f"Summary image will be saved as: {args.output_filename} in {args.results_dir}")
    ...
    if config_data:
        ...
        if not image_configurations:
            ...
            create_segmentation_summary_with_consistency_coloring(
